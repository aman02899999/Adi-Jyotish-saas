...
export function TwoFactorSettings({ apiPrefix, initialEnabled, description }: { apiPrefix: string; initialEnabled: boolean; description: string }) {
  ...
  async function startEnrollment() {
    setBusy(true);
    setError("");
    const response = await fetch(`${apiPrefix}/enroll`, { method: "POST" });
  ...
  async function confirmEnrollment() {
    setBusy(true);
    setError("");
    const response = await fetch(`${apiPrefix}/confirm`, {
      method: "POST",
  ...
  async function disable() {
    setBusy(true);
    setError("");
    const response = await fetch(`${apiPrefix}/disable`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    return (
      <section className="settings-card two-factor-card">
        ...
        <div className="two-factor-enroll">
          {/* eslint-disable-next-line @next/next/no-img-element -- a data: URL QR code, not a next/image-eligible remote asset */}
  ...
  return (
    <section className="settings-card two-factor-card">
      ...
      <p className="two-factor-hint">{enabled ? description : "Add a second step at sign-in using an authenticator app."}</p>
      ...
      {disabling && (
        <div className="two-factor-disable">
          <label className="field"><span>Enter your 6-digit code or a backup code</span><input value={disableCode} onChange={(event) => setDisableCode(event.target.value.trim())} placeholder="000000" /></label>
          {error && <p className="admin-auth-error" role="alert">{error}</p>}
          <div className="modal-actions">
            <button type="button" className="button button--ghost" onClick={() => { setDisabling(false); setError(""); }}>Cancel</button>
            <button className="button" disabled={busy || !disableCode} onClick={disable}>{busy ? "Turning off…" : "Turn off two-factor auth"}</button>
          </div>
        </div>
      )}
    </section>
