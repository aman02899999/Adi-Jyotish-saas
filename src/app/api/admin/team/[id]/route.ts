...
export async function PUT(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  const body=await request.json() as {role?:string;active?:boolean};const role=body.role&&await roleSlugExists(body.role)?body.role:existing.role;const active=body.active??existing.active;
