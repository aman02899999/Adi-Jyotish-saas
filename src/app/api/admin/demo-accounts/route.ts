...
import { createGoTrueUser, findGoTrueUserByEmail, updateGoTrueUser } from "@/lib/gotrue-admin";
...
const DEMO_PRACTITIONER_BIO =
  "A demo practitioner profile for walkthroughs — bookings and chat behave exactly like a real profile (payouts are disabled for this account).";
 ...
 * practitioner-portal.ts). Direct provider-admin calls (not the public self-serve/invite HTTP
...
export async function POST() {
  ...
  async function upsertAuthUser(email: string, displayName: string) {
    if (cutover) {
      // GoTrue has no getUserByEmail that throws on a miss, so the two cases are branches
      // rather than a try/catch: a not-found is an expected outcome, not an error.
      const existing = await findGoTrueUserByEmail(email);
      if (existing) {
        await updateGoTrueUser(existing.uid, { password, emailConfirm: true });
        return existing.uid;
      }
      return (await createGoTrueUser({ email, password, name: displayName })).uid;
    }
    try {
  ...
  const admins = await Promise.all(ADMIN_SEEDS.map(async (seed) => {
    ...
    if (cutover) {
      await upsertDemoAdminInSupabase({ id: uid, name: seed.name, email: seed.email });
    } else {
  ...
  const practitioners = await Promise.all(PRACTITIONER_SEEDS.map(async (seed) => {
    const uid = await upsertAuthUser(seed.email, seed.name);
    if (cutover) {
      if (await demoPractitionerExistsInSupabase(seed.slug)) {
        await updateDemoPractitionerInSupabase(seed.slug, uid);
      } else {
        await insertDemoPractitionerInSupabase({ slug: seed.slug, name: seed.name, email: seed.email, uid, bio: DEMO_PRACTITIONER_BIO });
        await seedDemoAvailabilityInSupabase(seed.slug);
      }
    } else {
  ...
  const members = await Promise.all(MEMBER_SEEDS.map(async (seed) => {
    const uid = await upsertAuthUser(seed.email, seed.name);
    const plan = proPlan?.key ?? "member";
    if (cutover) {
      if (await demoMemberExistsInSupabase(uid)) {
        await updateDemoMemberInSupabase({ id: uid, name: seed.name, email: seed.email, plan });
      } else {
        await insertDemoMemberInSupabase({ id: uid, name: seed.name, email: seed.email, plan });
      }
    } else {
      const memberRef = db.collection("members").doc(uid);
      const memberSnap = await memberRef.get();
      if (memberSnap.exists) {
        await memberRef.update({ name: seed.name, email: seed.email, active: true, isDemoAccount: true, plan, updatedAt: now });
      } else {
        await memberRef.set({
          name: seed.name,
          email: seed.email,
    ...
    if (proPlan) {
      if (cutover) {
        await upsertDemoSubscriptionInSupabase(uid, proPlan.id);
      } else {
