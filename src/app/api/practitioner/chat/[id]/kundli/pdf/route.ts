...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const isAiPowered = isSupabaseCutoverActive()
      ? Boolean((await getPractitionerAttributionInSupabase(practitioner.id))?.isAiPowered)
      : Boolean(((await db.collection("practitioners").doc(practitioner.id).get()).data() as { isAiPowered?: boolean } | undefined)?.isAiPowered);
