...
1. **Everything the guard can see is ported except the gemstone store.** The exact list is `src/lib/cutover-coverage.allowlist.ts`: every route or page, and every exported library function, that touches Firestore without consulting `isSupabaseCutoverActive()`. `cutover-coverage.test.ts` recomputes it on every CI run and fails on anything new that is not listed, and on anything listed that has since been ported, so the list can only shrink. It started at 48 entries and now holds 9: eight in the gemstone store (wishlist ×4, recommendations, the admin review and product lists, the catalogue seed), which is closed to customers, and `seedPractitioners`, which stays on Firestore by design. Phase 4 cannot start until each entry is ported or carries a reason to stay, so either port the store's eight or record that it stays closed through cutover. The guard is textual: it sees direct `db.collection`/`runTransaction`/`batch`/`getAll`/`collectionGroup`/`recursiveDelete` calls, not Firebase Storage or Auth, so the phase-4 staging run is still the proof. Each ported module keeps its SQL in a `<module>-supabase.ts` twin with an integration test against a real Postgres; `wallet.ts` is the worked example for anything that handles money and `chat.ts` for anything that takes a lock. The copy script also had gaps that only real data exposed; each has a test in `firestore-copy.integration.test.ts`: gift cards keyed only by document id, and experiments whose parent document never exists (copied as id-only rows via `includeMissingDocs`).
