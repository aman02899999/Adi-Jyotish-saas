...
Applying all ten migrations to an empty PostgreSQL 18.4 database yields **66 tables, 359 distinct column names, 159 indexes, 60 foreign keys, RLS enabled on all 66**, and re-applies idempotently.
...
9. **Unfreeze and watch.** Keep the Firebase project running and the Firestore data untouched for a full billing cycle. `auth_uid_map` is the audit trail of who became whom — drop it only after that.
