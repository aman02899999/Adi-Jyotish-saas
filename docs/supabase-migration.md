...
| `supabase/migrations/0014_horoscope_cache_key.sql` | drops the unique index on `daily_horoscopes (sign, date)`: a Monday's daily reading and that week's reading share both, so whichever came second could not be copied or cached (the id already keys each reading) | applied clean, twice; `content-cache-supabase.integration.test.ts` fails with the index restored |
| `supabase/migrations/0015_numerology_reading_fields.sql` | adds `life_path_number`, `destiny_number` and `narrative` to `numerology_readings`: the copy drops fields with no column, so every copied reading would have lost its result | applied clean, twice; `firestore-copy.integration.test.ts` and `readings-supabase.integration.test.ts` fail without it |
...
1. **Not everything is ported yet; the exact list is `src/lib/cutover-coverage.allowlist.ts`**: every route or page, and every exported library function, that touches Firestore without consulting `isSupabaseCutoverActive()`. `cutover-coverage.test.ts` recomputes it on every CI run and fails on anything new that is not listed, and on anything listed that has since been ported, so the list can only shrink. It started at 48 entries and fell to 9, at which point two blind spots in the detector turned up and 21 more became visible: functions that use a module-level handle (`const reviewsCol = db.collection(…)`, then `reviewsCol.doc(id)` with no `db.` in the function), and exports wrapped in `unstable_cache(…)`. The remaining entries are the CMS (custom pages, homepage hero, footer, promo banner), the homepage stats, the gemstone store, and `seedPractitioners`, which stays on Firestore by design. Phase 4 cannot start until each entry is ported or carries a reason to stay. The guard is textual: it sees `db.collection`/`runTransaction`/`batch`/`getAll`/`collectionGroup`/`recursiveDelete` calls, direct or through a module-level handle, a local helper or a cache wrapper; it does not see Firebase Storage or Auth, so the phase-4 staging run is still the proof. Each ported module keeps its SQL in a `<module>-supabase.ts` twin with an integration test against a real Postgres; `wallet.ts` is the worked example for anything that handles money and `chat.ts` for anything that takes a lock. The copy script also had gaps that only real data exposed; each has a test in `firestore-copy.integration.test.ts`: gift cards keyed only by document id, and experiments whose parent document never exists (copied as id-only rows via `includeMissingDocs`).
